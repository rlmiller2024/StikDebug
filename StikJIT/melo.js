...
function attach(breakpointcount) {
    ...
    while (validBreakpoints < breakpointcount) {
        ...
        if (!tid || !pc || !x0) {
            log(`Failed to extract registers: tid=${tid}, pc=${pc}, x0=${x0}`);
            continue;
        }
        
        const pcNum = littleEndianHexStringToNumber(pc);
        const x0Num = littleEndianHexStringToNumber(x0);
        log(`tid = ${tid}, pc = ${pcNum.toString(16)}, x0 = ${x0Num.toString(16)}`);
        ...
        let prepareJITPageResponse = prepare_memory_region(jitPageAddress, x0Num);
